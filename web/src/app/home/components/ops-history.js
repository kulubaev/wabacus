      ...
      <div className="ops-container">
        <header className="ops-line">
          ...
          <div className="ops-export-btn">
            ...
            <button
              ...
              <svg version="1.1" height="15px"  width="15px" x="0px" y="0px" viewBox="0 0 1000 1000" enableBackground="new 0 0 1000 1000" >
        ...
        <ul className="ops-list">
          ...
          <li className="ops-entry ops-entry-header">
						{
							columns.map((col, i)  => <span key={`${i}-col`}  className={`ops-${col.key}`}>{col.label}</span>)
						}
					</li>
          {list.map((item, i) => (
            <li key={`${i}-op`} className="ops-entry">
						{
							columns.map((col, i)  => <span key={`${i}-col`} className={`ops-${col.key}`}>{item[col.key]}</span>)
						}
            </li>
          ))}
        </ul>
